...
def send_email_change_confirmation(request, user, new_email, old_email):
    ...
    confirm_url = request.build_absolute_uri(urls.reverse("confirm_email_change", args=[token]))
    context = {"user": user, "confirm_url": confirm_url, "site": Site.objects.get_current()}
